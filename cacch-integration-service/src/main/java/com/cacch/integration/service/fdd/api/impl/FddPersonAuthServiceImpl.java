...
public class FddPersonAuthServiceImpl implements IFddPersonAuthService {
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS, readOnly = true, timeout = 10, rollbackFor = Exception.class)
    public FddPersonAuthDO findLatestPending(String internalCompanyName, String idNumber, String mobile) {
        if (!StringUtils.hasText(internalCompanyName)
                || !StringUtils.hasText(idNumber)
                || !StringUtils.hasText(mobile)) {
            return null;
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS, readOnly = true, timeout = 10, rollbackFor = Exception.class)
    public FddPersonAuthDO findLatestFailed(String internalCompanyName, String idNumber, String mobile) {
        if (!StringUtils.hasText(internalCompanyName)
                || !StringUtils.hasText(idNumber)
                || !StringUtils.hasText(mobile)) {
            return null;
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS, readOnly = true, timeout = 10, rollbackFor = Exception.class)
    public boolean hasFailedHistory(String internalCompanyName, String idNumber, String mobile) {
        if (!StringUtils.hasText(internalCompanyName)
                || !StringUtils.hasText(idNumber)
                || !StringUtils.hasText(mobile)) {
            return false;
    ...
    public FddPersonAuthDO insertPending(FddPersonAuthDO record) {
        if (record == null
                ...
                || !StringUtils.hasText(record.getMobile())
            ...
            throw new BizException(ResultCode.PARAM_MISSING, "个人认证记录必填字段缺失（含 mobile）");
        }
        record.setAuthStatus(FddAuthStatusEnum.PENDING.getCode());
        fddPersonAuthMapper.insert(record);
        log.info("【{}】新增 PENDING 成功, id={}, internalCompany={}, idNumber={}, mobile={}, transactionNo={}",
                LOG_BIZ, record.getId(), record.getInternalCompanyName(),
                maskIdNumber(record.getIdNumber()), maskMobile(record.getMobile()), record.getTransactionNo());
        return record;
    }
    ...
    public FddPersonAuthDO insertSuccessFromRemote(FddPersonAuthDO record) {
        if (record == null
            ...
            throw new BizException(ResultCode.PARAM_MISSING, "个人认证同步记录必填字段缺失（含 mobile）");
        }
        ...
        fddPersonAuthMapper.insert(record);
        log.info("【{}】同步 SUCCESS 成功, id={}, internalCompany={}, idNumber={}, mobile={}, fddAccountId={}",
                LOG_BIZ, record.getId(), record.getInternalCompanyName(),
                maskIdNumber(record.getIdNumber()), maskMobile(record.getMobile()), record.getFddAccountId());
        return record;
    }
