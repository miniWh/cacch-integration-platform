...
-- V23: Moka 自定义角色中间表
--   对接 Moka 开放平台角色查询接口
--   ( https://www.mokahr.com/docs/api/?shell#-75 )
...
CREATE TABLE IF NOT EXISTS t_integration_moka_role (
    ...
    id                    BIGINT       NOT NULL,
    -- ========== Moka 业务字段 ==========
    role_id               INTEGER      NOT NULL,   -- Moka 自定义角色 ID（业务唯一键，UNIQUE；Moka roleId）
    role_name             VARCHAR(128),             -- Moka 角色名称（Moka roleName / name）
    description           VARCHAR(500),             -- Moka 角色描述（Moka description）
    -- ========== 审计字段 ==========
...
COMMENT ON COLUMN t_integration_moka_role.role_id          IS 'Moka 自定义角色 ID（业务唯一键，UNIQUE；对应 Moka API roleId）';
COMMENT ON COLUMN t_integration_moka_role.role_name         IS 'Moka 角色名称（对应 Moka API roleName / name）';
