...
| 按行精确匹配 | 每个资料项目行对应共享盘一个目录；同负责人下以 **IPDP 项目编号** 区分多个 IPDP |
| 最新版本优先 | 同目录多版本时，仅同步版本号最大者 |
| 按名称 + 项目编号匹配 | 路径拼接、业务关联、幂等键使用 **登记负责人 + IPDP 名称 + IPDP 项目编号 + 资料项目名称** |
| **查表读 + CAP4 写** | **读取资料列表走 OA 库 SQL**；附件上传与 field0218 绑定走 **REST + CAP4 batch-update** |
| 幂等不重复 | 中间表已记录 SUCCESS 的 `(登记负责人, IPDP名称, IPDP项目编号, 资料项目名称)` 且文件创建时间未变则不再上传 |
...
| IPDP 名称 | **field0160** | 10% 环丙氟虫胺可分散液剂 | 共享盘 L2 括号外名称；幂等键 |
| IPDP 项目编号 | **field0164** | IPDP-202508-089 | 共享盘 L2 括号内编号；幂等键；**区分同负责人下多个 IPDP** |
...
> **匹配规则（已确认）**：共享盘路径、中间表幂等、业务唯一判断按 **登记负责人 + IPDP 名称 + IPDP 项目编号（field0164）+ 资料项目名称**；L2 目录括号中英文均可，**默认中文括号**。
...
| 项目编号位置 | **最后一对**括号内为 field0164；IPDP 名称段可含配方括号如 `(6+15)` |
| 示例 | `21%环丙氟虫胺·螺虫乙酯可分散液剂 (6+15)（IPDP-202605-107）` |
| 扫描匹配 | **读取 L2 目录时**按 field0164 与 OA 预加载索引匹配，通过后才进入 L3 读文件 |
| 项目编号 | 括号内填写 OA `field0164` 原文，不限制格式；入库以 field0164 为准 |
| 旧结构 | 仅 `{IPDP名称}` 的 L2 目录**不再扫描**（无法区分同负责人多项目） |

**路径拼接公式**：

```
\\192.168.1.8\国内登记资料\{field0223}\{field0160（field0164）}\{field0214}\
```

示例：

```
\\192.168.1.8\国内登记资料\杨燕玲\10%环丙氟虫胺可分散液剂（IPDP-202508-089）\产品概述\
```
...
| OA | 登记信息主表 | `formmain_4070` | field0223 负责人、field0160 IPDP名称、**field0164 项目编号** |
...
```
(登记负责人, IPDP名称, IPDP项目编号, 资料项目名称)
```

比对「是否已同步」时 additionally 使用共享盘文件 **创建时间**（`file_created_at`）。

| 条件 | 行为 |
|------|------|
| 中间表已有上述键且 `sync_status=SUCCESS`，`file_created_at` 未变 | **SKIP**，不重复上传 |
| 共享盘文件创建时间变化（同资料项换新文件） | **重新上传**并更新中间表 |
...
FOR EACH 子表行 row IN formson_5464:
    ...
    ipdpName      = formmain_4070.field0160    ← IPDP 名称（L2 括号外）
    ipdpProjectNo = formmain_4070.field0164    ← IPDP 项目编号（L2 括号内）
    itemName      = row.field0214              ← 资料项目
    itemRowId     = row.id                     ← OA 子表行 ID（写附件定位用）

    sharePath = {root}/{ownerName}/{ipdpName（ipdpProjectNo）}/{itemName}/
    file      = pickLatestFinalVersion(sharePath)
...
| IPDP 名称 | field0160 | 共享盘 L2 括号外；幂等键 |
| IPDP 项目编号 | field0164 | 共享盘 L2 括号内；幂等键；**避免同负责人多 IPDP 误绑** |
| 资料项目名称 | field0214 | 共享盘 L3；幂等键；匹配子表行 |
| 子表行 ID | row.id | CAP4 batch-update 定位 formson_5464 具体行 |
...
| field0164 项目编号 | IPDP-202508-089 |
| field0214 资料项目 | 产品概述 |
| 拼出共享盘路径 | `\\192.168.1.8\国内登记资料\杨燕玲\10%环丙氟虫胺可分散液剂（IPDP-202508-089）\产品概述\` |
...
中间表幂等键：
  (field0223, field0160, field0164, field0214) + file_created_at
```
...
| 情况 | 处理 |
|------|------|
| 同负责人下多个 IPDP 名称相同、项目编号不同 | L2 必须含 **field0164 项目编号**；匹配时 **名称 + 编号** 联合判断 |
