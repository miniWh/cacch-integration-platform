...
| 项 | 说明 |
|----|------|
| 格式 | `{field0160 IPDP名称}（{field0164 项目编号}）` |
| 括号 | 中文 `（）` 与英文 `()` **均支持**；程序拼接路径时**默认中文括号** |
| 项目编号 | 括号内填写 OA `formmain_4070.field0164` 的值，**不限制格式**；同步入库与幂等键均以 field0164 为准 |
| 示例 | `21%环丙氟虫胺·螺虫乙酯可分散液剂（IPDP-202501-010）` |
