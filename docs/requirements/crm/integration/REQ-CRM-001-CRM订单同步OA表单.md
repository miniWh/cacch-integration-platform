...
**目标**：定时从勤策CRM按 `modify_time` 拉取已审批通过的订单及明细，全量存入PG库（仅新增不更新）；OA同步时经独立的 CRM↔OA 人员映射解析业务员/发起人，再以**明细**为粒度映射并发起致远OA表单（一条明细一个表单）。采集与OA同步通过PG库解耦。
...
| 系统 | 模块/接口 | 说明 |
|------|-----------|------|
| PG数据库 | 订单主表 | 全量存储CRM订单查询接口返回的全部字段；仅新增不更新 |
| PG数据库 | 订单明细子表 | 全量存储CRM订单明细；按order_id关联主表；OA同步状态挂在明细上 |
| PG数据库 | CRM↔OA人员映射表 | 独立存储 crm_employee_id / emp_code / oa_user_id 等，供同步复用 |
...
**存储原则**：
- CRM返回什么字段，PG库就存什么字段
- 字段名按CRM接口返回的原始字段名存储（如 `id`、`name`、`customer`、`modify_time` 等）
- 嵌套对象（如 `customer`、`owner`、`department`）展开存储或以JSONB存储
- **仅新增不更新**：主表按订单编号、明细按明细ID判重，已存在则跳过
...
| emp_code | 文本 | CRM员工账号接口返回的 emp_code |
| oa_user_id | 文本 | OA按 code 查询得到的人员ID |
...
- 未命中则走 CRM→OA 解析链路，成功后 **UPSERT** 落库
- 人员映射允许更新（与订单「仅新增不更新」分离）

> **说明**：6.2/6.3 为CRM接口返回的全部字段清单（全量存储到PG库）。6.4/6.5 为现有ESB规则代码中实际使用的字段映射关系（已按本期确认调整人员/销售公司取值）。6.6 为OA表单流程的请求参数结构。6.7 的同步状态字段挂在**明细**上。6.8 为独立人员映射表。
>
> **字段映射分类**：
> - **CRM直传字段**：从CRM订单/明细数据中直接取值（含嵌套对象取label/name/value）；客户名称取 `customer.name`
...
| V4 | 订单数据全量存储 | 自动 | PG库主表/明细字段与CRM接口返回字段对应完整 |
...
| Q19 | CRM员工账号查询 / OA按编码取人员 完整URL | 开发方 | ✅ 已确认：CRM `POST /api/employee/v3/queryEmployee/...`（[文档](https://api.waiqin365.com/SERVER/sysapp/employee_v3.html#%E6%9F%A5%E8%AF%A2%E5%91%98%E5%B7%A5%E5%B8%90%E5%8F%B7)）；OA `GET /seeyon/rest/orgMembers/code/{code}`（[文档](https://open.seeyoncloud.com/seeyonapi/728/732.html#%E6%8C%89%E7%BC%96%E7%A0%81%E5%8F%96%E4%BA%BA%E5%91%98%E4%BF%A1%E6%81%AF)） |
