...
| F6 | 发起OA表单流程 | 在致远OA中自动发起对应的表单流程，**无需配置审批人**；**业务员(field0006)与流程发起人均取 CRM→OA 人员映射结果**；**销售公司(field0003)取 field_HVwgS__c.label**；`formson_2818`（费用利润子表）**本期不传** | 必选 |
...
逐条明细处理：
    ...
    │   ├─ field0003 = field_HVwgS__c.label
    │   └─ 不传 formson_2818
...
| 2 | field_HVwgS__c.label | field0003 | 销售公司 | 取嵌套对象label | CRM自定义字段；选单位 |
...
> - **field0003(销售公司)** 取 CRM `field_HVwgS__c.label`
...
> - **销售公司**：field0003 ← CRM `field_HVwgS__c.label`
