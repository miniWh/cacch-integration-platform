...
| 索引占位符 | `wecom.apps` 为 List 结构，secret 用 `WECOM_APPS_0_SECRET` / `WECOM_APPS_1_SECRET` 逐项注入 |
...
| `WECOM_APPS_0_CORPID` | `wecom.apps[0].corpid` | 企微自建应用企业 ID | 走默认 |
| `WECOM_APPS_0_SECRET` | `wecom.apps[0].secret` | **企微自建应用 Secret** | 对应应用鉴权不可用 |
| `WECOM_APPS_1_CORPID` | `wecom.apps[1].corpid` | 企微通讯录应用企业 ID | 走默认 |
| `WECOM_APPS_1_SECRET` | `wecom.apps[1].secret` | **企微通讯录应用 Secret** | 对应应用鉴权不可用 |
...
应用由 systemd 拉起（`/home/zhf/app/startup.sh`），推荐将环境变量写入 `startup.sh`（`export` 方式，重启即生效）：

```bash
# 编辑启动脚本，追加（示例）
cat >> /home/zhf/app/startup.sh <<'EOF'

# ===== 敏感信息环境变量（严禁提交版本库/写日志） =====
export DB_PASSWORD='<PG 密码>'
export REDIS_PASSWORD='<Redis 密码>'
export WECOM_APPS_0_SECRET='<企微自建应用 Secret>'
export WECOM_APPS_1_SECRET='<企微通讯录应用 Secret>'
...
```
DB_PASSWORD=...;REDIS_PASSWORD=...;WECOM_APPS_0_SECRET=...;WECOM_APPS_1_SECRET=...;CRM_APP_KEY=...;OA_REST_PASSWORD=...;OA_DB_PASSWORD=...;SHARE_DRIVE_PASSWORD=...;FDD_APP_SECRET=...;TENCENT_MEETING_SECRET_ID=...;TENCENT_MEETING_SECRET_KEY=...
```
...
| `WECOM_APPS_0_SECRET` / `WECOM_APPS_1_SECRET` | 启动正常，对应企微应用鉴权不可用 | 启动正常，对应企微应用鉴权不可用 |
